...
public class CmdbProvider implements CmdbReader, CmdbWriter {
    ...
    public void load() {
        ...
        if (labelNames == null || labelNames.isEmpty()) {
            log.warn("[LOAD] init label names failed!");
        } else {
    ...
    public class CmdbLabelTask implements Runnable {
        ...
        public void run() {
            ...
            log.debug("LABEL-TASK {}", "start dump.");
            ...
            try {
                ...
                Set<String> labelNames = cmdbService.getLabelNames();
                if (labelNames == null || labelNames.isEmpty()) {
                    log.warn("CMDB-LABEL-TASK {}", "load label names failed!");
                } else {
                    for (String labelName : labelNames) {
                        // If get null label, it's still ok. We will try it later when we meet this label:
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("LABEL-TASK {}", "got label map:" + JSON.toJSONString(tmpLabelMap));
                    }
                ...
                log.error("CMDB-LABEL-TASK {}", "dump failed!", e);
            } finally {
    ...
    public class CmdbDumpTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                log.debug("DUMP-TASK {}", "start dump.");
                ...
                log.error("DUMP-TASK {}", "dump failed!", e);
            } finally {
    ...
    public class CmdbEventTask implements Runnable {
        ...
        public void run() {
            try {
                log.debug("EVENT-TASK {}", "start dump.");
                if (cmdbService == null) {
                    return;
                }
                ...
                if (log.isDebugEnabled()) {
                    log.debug("EVENT-TASK {}", "got events size:" + ", events:" + JSON.toJSONString(events));
                }
                ...
                log.error("CMDB-EVENT {}", "event task failed!", e);
            } finally {
