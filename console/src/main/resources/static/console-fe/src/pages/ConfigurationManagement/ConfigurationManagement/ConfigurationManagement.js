...
class ConfigurationManagement extends React.Component {
  ...
  setIsCn() {
    this.setState({ isCn: sessionStorage.getItem(LANGUAGE_KEY) === 'zh-CN' });
  }
  ...
  toggleShowQuestionnaire(value) {
    if (value) {
      sessionStorage.setItem('acm_questionnaire', 1);
    } else {
      sessionStorage.removeItem('acm_questionnaire');
    }
  }

  checkQuestionnaire() {
    const acm_questionnaire = sessionStorage.getItem('acm_questionnaire');
  ...
  exportData() {
    ...
    const { accessToken = '' } = JSON.parse(sessionStorage.token || '{}');
