...
public class CredentialWatcher {
    ...
    public void stop() {
        ...
        log.info("[{}] {} is stopped", appName, this.getClass().getSimpleName());
    }
    ...
    private void loadCredential(boolean init) {
        ...
        if (propertyPath == null) {
            ...
            if (propertyPath == null || propertyPath.isEmpty()) {
                ...
                if (propertyPath == null || propertyPath.isEmpty()) {
                    ...
                    if (logWarn) {
                        log.info("[{}] Defined credential file: -Dspas.identity={}", appName, propertyPath);
                    }
                }
            } else {
                if (logWarn) {
                    log.info("[{}] Load credential file from classpath: {}", appName,
        ...
        if (propertiesIS == null) {
            ...
            if (accessKey == null && secretKey == null) {
                if (logWarn) {
                    log.info("{} No credential found", appName);
                }
                return;
            }
        } else {
            ...
            try {
                ...
                log.error("[26] Unable to load credential file, appName:" + appName
                ...
                try {
                    ...
                    log.error("[27] Unable to close credential file, appName:" + appName
                        + "Unable to close credential file " + propertyPath, e);
                }
            }

            if (logWarn) {
                log.info("[{}] Load credential file {}", appName, propertyPath);
            }
        ...
        if (!credential.valid()) {
            log.warn("[1] Credential file missing required property {} Credential file missing {} or {}",
