...
public class ServerListManager {
    ...
    public synchronized void start() throws NacosException {
        ...
        if (serverUrls.isEmpty()) {
            log.error("[init-serverlist] fail to get NACOS-server serverlist! env: {}, url: {}", name,
                addressServerUrl);
    ...
    Iterator<String> iterator() {
        if (serverUrls.isEmpty()) {
            log.error("[{}] [iterator-serverlist] No server address defined!", name);
        }
    ...
    class GetServerListTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                log.error("[" + name + "][update-serverlist] failed to update serverlist from address server!",
                    e);
            }
        }
    }
    ...
    private void updateIfChanged(List<String> newList) {
        if (null == newList || newList.isEmpty()) {
            log.warn("[update-serverlist] current serverlist from address server is empty!!!");
            return;
        }
    ...
    private List<String> getApacheServerList(String url, String name) {
        try {
            ...
            if (HttpURLConnection.HTTP_OK == httpResult.code) {
                ...
                log.error("[check-serverlist] error. addressServerUrl: {}, code: {}", addressServerUrl,
                    httpResult.code);
                return null;
            }
        } catch (IOException e) {
            log.error("[check-serverlist] exception. url: " + url, e);
            return null;
        }
    }
