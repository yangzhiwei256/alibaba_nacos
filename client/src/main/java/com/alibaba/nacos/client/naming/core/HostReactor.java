...
public class HostReactor {
    ...
    public ServiceInfo processServiceJSON(String json) {
        ...
        if (oldService != null) {
            ...
            if (oldService.getLastRefTime() > serviceInfo.getLastRefTime()) {
                log.warn("out of date data received, old-t: " + oldService.getLastRefTime()
            ...
            if (newHosts.size() > 0) {
                changed = true;
                log.info("new ips(" + newHosts.size() + ") service: "
                    + serviceInfo.getKey() + " -> " + JSON.toJSONString(newHosts));
            }

            if (remvHosts.size() > 0) {
                changed = true;
                log.info("removed ips(" + remvHosts.size() + ") service: "
                    + serviceInfo.getKey() + " -> " + JSON.toJSONString(remvHosts));
            }

            if (modHosts.size() > 0) {
                changed = true;
                log.info("modified ips(" + modHosts.size() + ") service: "
            ...
            changed = true;
            log.info("init new ips(" + serviceInfo.ipCount() + ") service: " + serviceInfo.getKey() + " -> " + JSON
        ...
        if (changed) {
            log.info("current ips:(" + serviceInfo.ipCount() + ") service: " + serviceInfo.getKey() +
            ...
            if (UPDATE_HOLD_INTERVAL > 0) {
                ...
                synchronized (serviceObj) {
                    try {
                        ...
                        log.error("[getServiceInfo] serviceName:" + serviceName + ", clusters:" + clusters, e);
                    }
                }
            }
        }
    ...
    public void updateServiceNow(String serviceName, String clusters) {
        ...
        try {
            ...
            log.error("[NA] failed to update serviceName: " + serviceName, e);
        } finally {
            if (oldService != null) {
        ...
        public void run() {
            try {
                ...
                if (!eventDispatcher.isSubscribed(serviceName, clusters) &&
                    ...
                    log.info("update task is stopped, service:" + serviceName + ", clusters:" + clusters);
                    return;
                }
                ...
                log.warn("[NA] failed to update serviceName: " + serviceName, e);
            }
