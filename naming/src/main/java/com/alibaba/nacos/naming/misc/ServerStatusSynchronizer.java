...
public class ServerStatusSynchronizer implements Synchronizer {
    ...
    public void send(final String serverIP, Message msg) {
        ...
        try {
            HttpClient.asyncHttpGet(url, null, params, new AsyncCompletionHandler() {
                ...
                public Integer onCompleted(Response response) throws Exception {
                    if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
                        log.warn("[STATUS-SYNCHRONIZE] failed to request serverStatus, remote server: {}",
                            serverIP);
            ...
            log.warn("[STATUS-SYNCHRONIZE] failed to request serverStatus, remote server: {}", serverIP, e);
        }
    }
