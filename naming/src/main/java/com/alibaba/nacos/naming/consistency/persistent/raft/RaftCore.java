...
public class RaftCore {
    ...
    private final Map<String, List<RecordListener>> listeners = new ConcurrentHashMap<>();

    private final ConcurrentMap<String, Datum> datums = new ConcurrentHashMap<>();
    ...
    public void init() throws Exception {
        ...
        log.info("cache loaded, datum count: {}, current term: {}", datums.size(), peers.getTerm());
        while (true) {
        ...
        initialized = true;
        log.info("finish to load data from disk, cost: {} ms.", (System.currentTimeMillis() - start));
    ...
    public void signalPublish(String key, Record value) throws Exception {
        ...
        try {
                ...
                HttpClient.asyncHttpPostLarge(url, Arrays.asList("key=" + key), content, new AsyncCompletionHandler<Integer>() {
                    ...
                    public Integer onCompleted(Response response) throws Exception {
                        if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
                            log.warn("[RAFT] failed to publish data to peer, datumId={}, peer={}, http code={}",
            ...
            if (!latch.await(UtilsAndCommons.RAFT_PUBLISH_TIMEOUT, TimeUnit.MILLISECONDS)) {
                ...
                log.error("data publish failed, caused failed to notify majority, key={}", key);
                throw new IllegalStateException("data publish failed, caused failed to notify majority, key=" + key);
            }

            long end = System.currentTimeMillis();
            log.info("signalPublish cost {} ms, key: {}", (end - start), key);
        } finally {
            OPERATE_LOCK.unlock();
        }
    }
            ...
            for (final String server : peers.allServersWithoutMySelf()) {
                ...
                HttpClient.asyncHttpDeleteLarge(url, null, JSON.toJSONString(json)
                    , new AsyncCompletionHandler<Integer>() {
                        ...
                        public Integer onCompleted(Response response) throws Exception {
                            if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
                                log.warn("[RAFT] failed to delete data from peer, datumId={}, peer={}, http code={}", key, server, response.getStatusCode());
                                return 1;
                            }
    ...
    public void onPublish(Datum datum, RaftPeer source) throws Exception {
        RaftPeer local = peers.local();
        if (datum.value == null) {
            log.warn("received empty datum");
            throw new IllegalStateException("received empty datum");
        }

        if (!peers.isLeader(source.ip)) {
            log.warn("peer {} tried to publish data but wasn't leader, leader: {}",
        ...
        if (source.term.get() < local.term.get()) {
            log.warn("out of date publish, pub-term: {}, cur-term: {}",
        ...
        log.info("data added/updated, key={}, term={}", datum.key, local.term);
    }
    ...
    public void onDelete(String datumKey, RaftPeer source) throws Exception {
        ...
        log.info("data removed, key={}, term={}", datumKey, local.term);
            ...
            for (final String server : peers.allServersWithoutMySelf()) {
                ...
                try {
                    HttpClient.asyncHttpPost(url, null, params, new AsyncCompletionHandler<Integer>() {
                        ...
                        public Integer onCompleted(Response response) throws Exception {
                            if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
                                log.error("NACOS-RAFT vote failed: {}, url: {}", response.getResponseBody(), url);
                                return 1;
                            }

                            RaftPeer peer = JSON.parseObject(response.getResponseBody(), RaftPeer.class);

                            log.info("received approve from peer: {}", JSON.toJSONString(peer));
    ...
    public class HeartBeat implements Runnable {
        ...
        public void sendBeat() throws IOException, InterruptedException {
            ...
            if (log.isDebugEnabled()) {
                log.debug("[RAFT] send beat with {} keys.", datums.size());
            }
            ...
            if (switchDomain.isSendBeatOnly()) {
                log.info("[SEND-BEAT-ONLY] {}", switchDomain.isSendBeatOnly());
            }
            ...
            if (log.isDebugEnabled()) {
                log.debug("raw beat data size: {}, size of compressed data: {}",
            ...
            for (final String server : peers.allServersWithoutMySelf()) {
                try {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("send beat to server " + server);
                    }
                    HttpClient.asyncHttpPostLarge(url, null, compressedBytes, new AsyncCompletionHandler<Integer>() {
                        @Override
                        public Integer onCompleted(Response response) throws Exception {
                            if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
                                log.error("NACOS-RAFT beat failed: {}, peer: {}",
                        ...
                        public void onThrowable(Throwable t) {
                            log.error("NACOS-RAFT error while sending heart-beat to peer: {} {}", server, t);
                            MetricsMonitor.getLeaderSendBeatFailedException().increment();
                        }
                    });
                } catch (Exception e) {
                    log.error("error while sending heart-beat to peer: {} {}", server, e);
                    MetricsMonitor.getLeaderSendBeatFailedException().increment();
                }
            }
    ...
    public RaftPeer receivedBeat(JSONObject beat) throws Exception {
        ...
        if (!switchDomain.isSendBeatOnly()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("[RAFT] received beat with {} keys, RaftCore.datums' size is {}, remote server: {}, term: {}, local term: {}",
            ...
            for (Object object : beatDatums) {
                ...
                try {
                    ...
                    log.info("get datums from leader: {}, batch size is {}, processedCount is {}, datums' size is {}, RaftCore.datums' size is {}"
                            ...
                            for (JSONObject datumJson : datumList) {
                                ...
                                try {
                                    ...
                                    if (oldDatum != null && datumJson.getLongValue("timestamp") <= oldDatum.timestamp.get()) {
                                        log.info("[NACOS-RAFT] timestamp is smaller than that of mine, key: {}, remote: {}, local: {}",
                                    ...
                                    if (newDatum == null || newDatum.value == null) {
                                        log.error("receive null datum: {}", datumJson);
                                        continue;
                                    }
                                    ...
                                    log.info("data updated, key: {}, timestamp: {}, from {}, local term: {}",
                                        newDatum.key, newDatum.timestamp, JSON.toJSONString(remote), local.term);

                                } catch (Throwable e) {
                                    log.error("[RAFT-BEAT] failed to sync datum from leader, datum: {}", newDatum, e);
                                } finally {
                                    OPERATE_LOCK.unlock();
                                }
                            }
                    ...
                    log.error("[NACOS-RAFT] failed to handle beat entry, key: {}", datumKey);
                }
            ...
            for (String deadKey : deadKeys) {
                try {
                    ...
                    log.error("[NACOS-RAFT] failed to remove entry, key={} {}", deadKey, e);
                }
            }
    ...
    public void listen(String key, RecordListener listener) {
        ...
        for (Datum datum : datums.values()) {
            ...
            try {
                ...
                log.error("NACOS-RAFT failed to notify listener", e);
            }
        }
    }
    ...
    public void loadDatum(String key) {
        try {
            ...
            log.error("load datum failed: " + key, e);
        }
    ...
    public class Notifier implements Runnable {
        ...
        private final ConcurrentHashMap<String, String> services = new ConcurrentHashMap<>(10 * 1024);

        private final BlockingQueue<Pair> tasks = new LinkedBlockingQueue<>(1024 * 1024);
        ...
        public void run() {
            ...
            while (true) {
                try {
                        ...
                        if (KeyBuilder.matchServiceMetaKey(datumKey) && !KeyBuilder.matchSwitchKey(datumKey)) {
                            ...
                            for (RecordListener listener : listeners.get(KeyBuilder.SERVICE_META_KEY_PREFIX)) {
                                try {
                                    ...
                                    log.error("[NACOS-RAFT] error while notifying listener of key: {}", datumKey, e);
                                }
                            }
                        }
                    }
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("[NACOS-RAFT] datum change notified, key: {}, listener count: {}", datumKey, count);
                    }
                } catch (Throwable e) {
                    log.error("[NACOS-RAFT] Error while handling notifying task", e);
                }
            }
        }
    }
