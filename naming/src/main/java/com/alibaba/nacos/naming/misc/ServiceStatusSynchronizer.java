...
public class ServiceStatusSynchronizer implements Synchronizer {
    ...
    public void send(final String serverIP, Message msg) {
        ...
        try {
            HttpClient.asyncHttpPostLarge(url, null, JSON.toJSONString(params), new AsyncCompletionHandler() {
                ...
                public Integer onCompleted(Response response) throws Exception {
                    if (response.getStatusCode() != HttpURLConnection.HTTP_OK) {
                        log.warn("[STATUS-SYNCHRONIZE] failed to request serviceStatus, remote server: {}", serverIP);
                        return 1;
                    }
                    return 0;
                }
            ...
            log.warn("[STATUS-SYNCHRONIZE] failed to request serviceStatus, remote server: " + serverIP, e);
        }
    ...
    public Message get(String serverIP, String key) {
        ...
        try {
            if (log.isDebugEnabled()) {
                log.debug("[STATUS-SYNCHRONIZE] sync service status from: {}, service: {}", serverIP, key);
            }
            ...
            log.warn("[STATUS-SYNCHRONIZE] Failed to get service status from " + serverIP, e);
            return null;
        }
