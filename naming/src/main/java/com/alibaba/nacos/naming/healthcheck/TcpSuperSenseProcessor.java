...
public class TcpSuperSenseProcessor implements HealthCheckProcessor, Runnable {
    ...
    private static final ScheduledExecutorService TCP_CHECK_EXECUTOR
    ...
    private static final ScheduledExecutorService NIO_EXECUTOR
        ...
        for (Instance ip : ips) {
            ...
            if (ip.isMarked()) {
                if (log.isDebugEnabled()) {
                    log.debug("tcp check, ip is marked as to skip health check, ip:" + ip.getIp());
                }
                continue;
            }

            if (!ip.markChecking()) {
                log.warn("tcp check started before last one finished, service: "
    ...
    public void run() {
        while (true) {
            try {
                ...
                log.error("[HEALTH-CHECK] error while processing NIO task", e);
            }
        }
    }
    ...
    private class TaskProcessor implements Callable<Void> {
        ...
        public Void call() {
            ...
            if (waited > MAX_WAIT_TIME_MILLISECONDS) {
                log.warn("beat task waited too long: " + waited + "ms");
            }
