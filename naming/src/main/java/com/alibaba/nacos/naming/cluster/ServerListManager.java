...
public class ServerListManager {
    ...
    private final Map<String, List<Server>> distroConfig = new ConcurrentHashMap<>();

    private final Map<String, Long> distroBeats = new ConcurrentHashMap<>(16);

    private final Set<String> liveSites = new HashSet<>();
    ...
    private List<Server> refreshServerList() {
        ...
        try {
            ...
            log.warn("failed to get config: " + getClusterConfFilePath(), e);
        }

        if (log.isDebugEnabled()) {
            log.debug("SERVER-LIST from cluster.conf: {}", result);
        }
        ...
        if (CollectionUtils.isEmpty(serverList)) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("SERVER-LIST from system variable: {}", result);
            }
        }
    ...
    public List<String> getHealthyServerIps() {
        if(CollectionUtils.isEmpty(healthyServers)){
            return Collections.EMPTY_LIST;
        }
        return healthyServers.stream().map(Server::getIp).collect(Collectors.toList());
    }
            ...
            for (Server s : list) {
                ...
                if (serverId.equals(newServerId)) {
                    if (s.isAlive() != server.isAlive() || s.getWeight() != server.getWeight()) {
                        log.warn("server beat out of date, current: {}, last: {}",
    ...
    private void requestOtherServerCleanInvalidServers(String serverIP) {
        ...
        try {
            ...
            log.warn("[DISTRO-STATUS-CLEAN] Failed to request to clean server status to " + serverIP, e);
        }
    }
        ...
        public void run() {
            try {
                ...
                if (CollectionUtils.isEmpty(refreshedServers)) {
                    log.warn("refresh server list failed, ignore it.");
                    return;
                }
        ...
        public void run() {
            try {
                ...
                if (!contains(NetUtils.localServer())) {
                    log.error("local ip is not in serverlist, ip: {}, serverlist: {}", NetUtils.localServer(), allServers);
                    return;
                }
                ...
                log.error("[SERVER-STATUS] Exception while sending server status", e);
            } finally {
    ...
    private void checkDistroHeartbeat() {
        ...
        log.debug("check distro heartbeat.");
        ...
        if (autoDisabledHealthCheck
            ...
            log.info("[NACOS-DISTRO] distro threshold restored and " +
        ...
        if (!CollectionUtils.isEqualCollection(healthyServers, newHealthyList)) {
            ...
            log.info("[NACOS-DISTRO] healthy server list changed, old: {}, new: {}",
                healthyServers, newHealthyList);
            if (switchDomain.isHealthCheckEnabled() && switchDomain.isAutoChangeHealthCheckEnabled()) {
                log.info("[NACOS-DISTRO] disable health check for {} ms from now on.", STABLE_PERIOD);
