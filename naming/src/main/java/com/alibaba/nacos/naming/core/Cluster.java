...
public class Cluster extends com.alibaba.nacos.api.naming.pojo.Cluster implements Cloneable {
    ...
    public void updateIPs(List<Instance> ips, boolean ephemeral) {
        ...
        if (updatedIPs.size() > 0) {
            for (Instance ip : updatedIPs) {
                ...
                if (ip.getWeight() != oldIP.getWeight()) {
                    // ip validation status updated
                    log.info("{} {SYNC} {IP-UPDATED} {}->{}", getService().getName(), oldIP.toString(), ip.toString());
                }
            }
        }
        ...
        if (newIPs.size() > 0) {
            log.info("{} {SYNC} {IP-NEW} cluster: {}, new ips size: {}, content: {}",
        ...
        if (deadIPs.size() > 0) {
            log.info("{} {SYNC} {IP-DEAD} cluster: {}, dead ips size: {}, content: {}",
    ...
    public void update(Cluster cluster) {
        ...
        if (!getHealthChecker().equals(cluster.getHealthChecker())) {
            log.info("[CLUSTER-UPDATE] {}:{}:, healthChecker: {} -> {}",
        ...
        if (!StringUtils.equals(sitegroup, cluster.getSitegroup())) {
            log.info("[CLUSTER-UPDATE] {}:{}, sitegroup: {} -> {}",
        ...
        if (isUseIPPort4Check() != cluster.isUseIPPort4Check()) {
            log.info("[CLUSTER-UPDATE] {}:{}, useIPPort4Check: {} -> {}",
