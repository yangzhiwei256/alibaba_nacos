...
public class Service extends com.alibaba.nacos.api.naming.pojo.Service implements Record, RecordListener<Instances> {
     ...
     */
    private final long pushCacheMillis = 0L;
    ...
    public void onChange(String key, Instances value) throws Exception {
        ...
        log.info("[NACOS-RAFT] datum is changed, key: {}, value: {}", key, value);
    ...
    public void updateIPs(Collection<Instance> instances, boolean ephemeral) {
        ...
        for (Instance instance : instances) {
            try {
                if (instance == null) {
                    log.error("[NACOS-DOM] received malformed ip: null");
                    continue;
                }
                ...
                log.error("[NACOS-DOM] failed to process ip: " + instance, e);
            }
        }
        ...
        log.info("[IP-UPDATED] namespace: {}, service: {}, ips: {}",
    ...
    public void update(Service vDom) {
        ...
        if (!StringUtils.equals(token, vDom.getToken())) {
            log.info("[SERVICE-UPDATE] service: {}, token: {} -> {}", getName(), token, vDom.getToken());
            token = vDom.getToken();
        }

        if (!ListUtils.isEqualList(owners, vDom.getOwners())) {
            log.info("[SERVICE-UPDATE] service: {}, owners: {} -> {}", getName(), owners, vDom.getOwners());
            owners = vDom.getOwners();
        }

        if (getProtectThreshold() != vDom.getProtectThreshold()) {
            log.info("[SERVICE-UPDATE] service: {}, protectThreshold: {} -> {}", getName(), getProtectThreshold(), vDom.getProtectThreshold());
            setProtectThreshold(vDom.getProtectThreshold());
        }

        if (resetWeight != vDom.getResetWeight().booleanValue()) {
            log.info("[SERVICE-UPDATE] service: {}, resetWeight: {} -> {}", getName(), resetWeight, vDom.getResetWeight());
            resetWeight = vDom.getResetWeight();
        }

        if (enabled != vDom.getEnabled().booleanValue()) {
            log.info("[SERVICE-UPDATE] service: {}, enabled: {} -> {}", getName(), enabled, vDom.getEnabled());
        ...
        log.info("cluster size, new: {}, old: {}", getClusterMap().size(), vDom.getClusterMap().size());
    ...
    public synchronized void recalculateChecksum() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("service to json: " + getServiceString());
        }
        ...
        try {
            ...
            try {
                ...
                result = new BigInteger(1, md5.digest((ipsString.toString()).getBytes(StandardCharsets.UTF_8))).toString(16);
            } catch (Exception e) {
                log.error("[NACOS-DOM] error while calculating checksum(md5)", e);
