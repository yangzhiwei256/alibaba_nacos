...
public class HealthCheckExtendProvider implements BeanFactoryAware {
    ...
    private final ServiceLoader<HealthCheckProcessor> processorLoader
        = ServiceLoader.load(HealthCheckProcessor.class);

    private final ServiceLoader<AbstractHealthChecker> checkerLoader
    ...
    private void loadExtend() {
        ...
        if(processorType.size() > origin.size()){
            ...
            log.debug("init health plugin : types=" + processorType);
        }
    }
