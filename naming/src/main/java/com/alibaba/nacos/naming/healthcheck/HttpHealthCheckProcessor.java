...
public class HttpHealthCheckProcessor implements HealthCheckProcessor {
    ...
    static {
        try {
            ...
            log.error("[HEALTH-CHECK] Error while constructing HTTP asynchronous client", e);
        }
    }
            ...
            try {
                ...
                if (ip.isMarked()) {
                    if (log.isDebugEnabled()) {
                        log.debug("http check, ip is marked as to skip health check, ip: {}" + ip.getIp());
                    }
                    continue;
                }

                if (!ip.markChecking()) {
                    log.warn("http check started before last one finished, service: {}:{}:{}",
