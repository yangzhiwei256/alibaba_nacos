        ...
        try {
            if (distroMapper.responsible(cluster.getService().getName()) &&
                ...
                if (log.isDebugEnabled()) {
                    log.debug("[HEALTH-CHECK] schedule health check task: {}", cluster.getService().getName());
                }
            }
        } catch (Throwable e) {
            log.error("[HEALTH-CHECK] error while process health check for {}:{}",
            ...
            if (!cancelled) {
                ...
                if (this.getCheckRTWorst() > 0
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("{}:{}@{}->normalized: {}, worst: {}, best: {}, last: {}, diff: {}",
