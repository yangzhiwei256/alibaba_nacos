...
public class DistroConsistencyServiceImpl implements EphemeralConsistencyService {
    ...
    private final LoadDataTask loadDataTask = new LoadDataTask();

    private final Map<String, CopyOnWriteArrayList<RecordListener>> listeners = new ConcurrentHashMap<>();
    ...
    public void load() throws Exception {
        ...
        while (serverListManager.getHealthyServers().size() <= 1) {
            Thread.sleep(1000L);
            log.info("current healthyServers: {} waiting server list init...", serverListManager.getHealthyServerIps());
        }
    ...
    public void onReceiveChecksums(Map<String, String> checksumMap, String server) {
        ...
        if (syncChecksumTasks.containsKey(server)) {
            ...
            log.warn("sync checksum task already in process with {}", server);
            return;
        }
        ...
        try {
            ...
            for (Map.Entry<String, String> entry : checksumMap.entrySet()) {
                if (distroMapper.responsible(KeyBuilder.getServiceName(entry.getKey()))) {
                    ...
                    log.error("receive responsible key timestamp of " + entry.getKey() + " from " + server);
                    // abort the procedure:
                    return;
                }
            ...
            if (log.isDebugEnabled()) {
                log.info("to remove keys: {}, to update keys: {}, source: {}", toRemoveKeys, toUpdateKeys, server);
            }
            ...
            try {
                ...
                log.error("get data from " + server + " failed!", e);
            }
        } finally {
    ...
    public boolean syncAllDataFromRemote(Server server) {
        ...
        try {
            ...
            log.error("sync full data from " + server + " failed!", e);
            return false;
        }
    }
    ...
    public void processData(byte[] data) throws Exception {
        if (data.length > 0) {
            ...
            for (Map.Entry<String, Datum<Instances>> entry : datumMap.entrySet()) {
                ...
                if (!listeners.containsKey(entry.getKey())) {
                    ...
                    if (switchDomain.isDefaultInstanceEphemeral()) {
                        ...
                        log.info("creating service {}", entry.getKey());
            ...
            for (Map.Entry<String, Datum<Instances>> entry : datumMap.entrySet()) {
                ...
                if (!listeners.containsKey(entry.getKey())) {
                    // Should not happen:
                    log.warn("listener of {} not found.", entry.getKey());
                    continue;
                }
                ...
                try {
                    ...
                    log.error("[NACOS-DISTRO] error while execute listener of key: {}", entry.getKey(), e);
                    continue;
                }
    ...
    public class Notifier implements Runnable {
        ...
        private final BlockingQueue<Pair> tasks = new LinkedBlockingQueue<Pair>(1024 * 1024);
        ...
        @Override
        public void run() {
            log.info("distro notifier started");
                ...
                try {
                    ...
                    for (RecordListener listener : listeners.get(datumKey)) {
                        ...
                        try {
                            ...
                            log.error("[NACOS-DISTRO] error while notifying listener of key: {}", datumKey, e);
                        }
                    }

                    if (log.isDebugEnabled()) {
                        log.debug("[NACOS-DISTRO] datum change notified, key: {}, listener count: {}, action: {}",
                            datumKey, count, action.name());
                    }
                } catch (Throwable e) {
                    log.error("[NACOS-DISTRO] Error while handling notifying task", e);
                }
            }
        }
    }
