...
public class ConfigService {
    ...
    static public boolean dump(String dataId, String group, String tenant, String content, long lastModifiedTs, String type) {
        ...
        try {
            ...
            log.error("[dump-exception] save disk error. " + groupKey + ", " + ioe.toString(), ioe);
    ...
    static public boolean dumpBeta(String dataId, String group, String tenant, String content, long lastModifiedTs,
        ...
        if (lockResult < 0) {
            log.warn("[dump-beta-error] write lock failed. {}", groupKey);
            return false;
        }
        ...
        try {
            ...
            log.error("[dump-beta-exception] save disk error. " + groupKey + ", " + ioe.toString(),
                ioe);
            return false;
        } finally {
            releaseWriteLock(groupKey);
    ...
    static public boolean dumpTag(String dataId, String group, String tenant, String tag, String content,
        ...
        if (lockResult < 0) {
            log.warn("[dump-tag-error] write lock failed. {}", groupKey);
            return false;
        }
        ...
        try {
            ...
            log.error("[dump-tag-exception] save disk error. " + groupKey + ", " + ioe.toString(),
                ioe);
            return false;
        } finally {
            releaseWriteLock(groupKey);
    ...
    static public boolean dumpChange(String dataId, String group, String tenant, String content, long lastModifiedTs) {
        ...
        try {
            ...
            log.error("[dump-exception] save disk error. " + groupKey + ", " + ioe.toString(),
                ioe);
            return false;
        } finally {
            releaseWriteLock(groupKey);
    ...
    static public void reloadConfig() {
        ...
        try {
            ...
            log.error("reload fail:" + AggrWhitelist.AGGRIDS_METADATA, e);
        }
        ...
        try {
            ...
            log.error("reload fail:" + SwitchService.SWITCH_META_DATAID, e);
        }
        ...
        for (Entry<String/* groupKey */, CacheItem> entry : CACHE.entrySet()) {
            ...
            try {
                ...
                if (!entry.getValue().md5.equals(loacalMd5)) {
                    log.warn("[md5-different] dataId:{},group:{}",
                        dataId, group);
                    diffList.add(groupKey);
                }
            } catch (IOException e) {
                log.error("getLocalConfigMd5 fail,dataId:{},group:{}",
                    dataId, group);
            }
        }
        long endTime = System.currentTimeMillis();
        log.warn("checkMd5 cost:{}; diffCount:{}", endTime - startTime,
            diffList.size());
        return diffList;
    }
