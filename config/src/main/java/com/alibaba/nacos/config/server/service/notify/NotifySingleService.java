...
public class NotifySingleService {
    ...
    static class NotifySingleTask extends NotifyTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                log.error("[notify-exception] target:{} dataid:{} group:{} ts:{}", target, getDataId(),
                    getGroup(), getLastModified());
                log.debug("[notify-exception] target:{} dataid:{} group:{} ts:{}",
                    new Object[] {target, getDataId(), getGroup(), getLastModified()}, e);
            }

            if (!this.isSuccess) {
                log.error("[notify-retry] target:{} dataid:{} group:{} ts:{}", target, getDataId(),
                    getGroup(), getLastModified());
                try {
                    ((ScheduledThreadPoolExecutor)executor).schedule(this, 500L, TimeUnit.MILLISECONDS);
                } catch (Exception e) { // 通知虽然失败，但是同时此前节点也下线了
                    log.warn("[notify-thread-pool] cluster remove node {}, current thread was tear down.", target,
                        e);
                }
            }
        }
    ...
    private void setupNotifyExecutors() {
        ...
        for (String ip : clusterIps) {
            ...
            if (null == executors.putIfAbsent(ip, executor)) {
                log.warn("[notify-thread-pool] setup thread target ip {} ok.", ip);
            }
        }
        ...
        for (Map.Entry<String, Executor> entry : executors.entrySet()) {
            ...
            if (!clusterIps.contains(target)) {
                ...
                log.warn("[notify-thread-pool] tear down thread target ip {} ok.", target);
            }
        }
    ...
    private final ConcurrentHashMap<String, Executor> executors = new ConcurrentHashMap<String, Executor>();
