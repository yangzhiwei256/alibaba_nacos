...
public class DumpService {
    ...
    public void init() {
        ...
        try {
            ...
            // 更新beta缓存
            log.info("start clear all config-info-beta.");
            ...
            // 更新Tag缓存
            log.info("start clear all config-info-tag.");
        ...
        if (!STANDALONE_MODE) {
            Runnable heartbeat = () -> {
                ...
                try {
                    ...
                    log.error("save heartbeat fail" + e.getMessage());
                }
            };
            ...
            log.warn("initialDelay:{}", initialDelay);
    ...
    private void dumpConfigInfo(DumpAllProcessor dumpAllProcessor)
        ...
        try {
            ...
            if (isAllDump) {
                log.info("start clear all config-info.");
                DiskUtil.clearAll();
                ...
                Runnable checkMd5Task = () -> {
                    ...
                    log.error("end checkMd5Task");
                };
                TimerTaskService.scheduleWithFixedDelay(checkMd5Task, 0, 12,
                    TimeUnit.HOURS);
            }
        } catch (IOException e) {
            log.error("dump config fail" + e.getMessage());
            throw e;
        } finally {
            if (null != fis) {
                try {
    ...
    private Boolean isQuickStart() {
        try {
            ...
            log.warn("isQuickStart:{}", isQuickStart);
        } catch (Exception e) {
            log.error("read application.properties wrong", e);
        }
        return isQuickStart;
    }
    ...
    private int getRetentionDays() {
        ...
        try {
            ...
            log.error("read nacos.config.retention.days wrong", nfe);
        }
    ...
    class MergeAllDataWorker extends Thread {
        ...
        private final List<ConfigInfoChanged> configInfoList;
