...
class PrintGetConfigResponeTask implements Runnable {
    @Override
    public void run() {
        log.info(ResponseMonitor.getStringForPrint());
    }
}
...
class PrintMemoryTask implements Runnable {
    ...
    public void run() {
        ...
        log.info("groupCount={}, subscriberClientCount={}, subscriberCount={}", groupCount, subClientCount,
            subCount);
...
class NotifyTaskQueueMonitorTask implements Runnable {
    ...
    public void run() {
        ...
        log.info("toNotifyTaskSize={}", size);
