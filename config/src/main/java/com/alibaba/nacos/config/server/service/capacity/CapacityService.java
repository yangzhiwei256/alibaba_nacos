...
public class CapacityService {
    ...
    public void init() {
        ...
        scheduledExecutorService.scheduleWithFixedDelay(new Runnable() {
            @Override
            public void run() {
                log.info("[capacityManagement] start correct usage");
                Stopwatch stopwatch = Stopwatch.createStarted();
                correctUsage();
                log.info("[capacityManagement] end correct usage, cost: {}s", stopwatch.elapsed(TimeUnit.SECONDS));
    ...
    private boolean insertGroupCapacity(String group, Integer quota, Integer maxSize, Integer maxAggrCount,
        ...
        try {
            ...
            log.warn("group: {}, message: {}", group, e.getMessage());
        }
        return false;
    }
